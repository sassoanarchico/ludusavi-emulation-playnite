    ...
    public class App
    {
        ...
        public (int, Output.Response?) Invoke(Invocation invocation)
        {
            ...
            try
            {
                ...
                if (code == 0)
                {
                    logger.Debug(string.Format("Ludusavi exited with {0} and valid JSON content (Success: data found/saved)", code));
                }
                else if (code == 1)
                {
                    logger.Debug(string.Format("Ludusavi exited with {0} and valid JSON content (Info: no data found, not an error)", code));
                }
                else
                {
                    logger.Warn(string.Format("Ludusavi exited with {0} and valid JSON content (Warning: possible error)", code));
                }
            }
            catch (Exception e)
            {
                ...
                if (code == 0)
                {
                    logger.Error(e, string.Format("Ludusavi exited with {0} but returned invalid JSON content - this is unexpected", code));
                }
                else
                {
