...
config-ask-when-multiple-games-are-running = Richiedi conferma quando più giochi sono in esecuzione

config-rpcs3-savedata-path = Percorso savedata RPCS3 personalizzato (cerca automaticamente i codici gioco PS3)

config-enable-emulated-game-automation = Abilita la creazione automatica di voci personalizzate Ludusavi per giochi emulati
config-emulated-game-title-format = Formato titolo per giochi emulati (supporta <name>, <platform>, <emulator>)
config-emulated-save-mappings = Mappature salvataggi emulatori
config-emulated-save-mappings-help = Aggiungi una riga per emulatore. Ogni riga definisce come trovare i salvataggi usando template come <romDir>, <romBase>, <romPath>, <emulatorDir>.
...
open-backup-directory = Apri cartella di backup
customize-in-ludusavi = Personalizza in Ludusavi
lock = Blocca
unlock = Sblocca
set-comment = Imposta commento
restore = Ripristina

## Giochi emulati
button-load-default-mappings = Carica preset emulatori predefiniti
button-clear-all-mappings = Cancella tutte le mappature
